...
\begin{document}
    ...
    \begin{section}{SYSTEM MODEL}
        ...
        \begin{subsection}{Information-Sharing Broadcast Model}
            ...
            Furthermore, we denote the latency time where $k$-th AP receives all the last broadcast information as \emph{Update Point}, which is defined as follows.
            \begin{definition}[Update Point]
                \begin{align}
                ...
                \end{align}
                where $D^{(p)}_{k,k'} \equiv 0$ for convenience. {\color{red}We assume that $D_k$ is bounded with $[0, \hat{D}_k]$ and follows some distribution in this interval summarized from statistical data, which is denotes as $\eta_k(d) \define \Pr\{D_k=d\}$.}
            \end{definition}
            ...
            {\color{red}In the next section, we will show that with the prior statistic knowledge about broadcast latency $\set{\eta_k|\forall k\in\apSet}$, we could establish exact cooperation among AP nodes under MDP frame work with analytical performance bound.}
        \end{subsection}
    \end{section}
