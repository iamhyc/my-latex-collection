    ...
    \begin{section}{PROBLEM FORMULATION}
        ...
        \begin{subsection}{The Optimization Problem}
            ...
            \begin{align}
                ...
                \nonumber\\
                =& g(\Stat_i) + \gamma \min_{\bar{\Policy}(\Stat_{i})} \sum_{\Stat_{i+1}}
                \Pr\{ \Stat_{i+1}|\Stat_{i}, \bar{\Policy}(\Stat_{i}) \} V(\Stat_{i+1}),
            \end{align}
            {\color{red}where $\bar{\Policy}(\Stat_{i}) \define [\bar{\Omega}_1(\Stat_{i}), \dots, \bar{\Omega}_K(\Stat_{i})]$; each $\bar{\Omega}_k(\Stat_{i})$ on $k$-th AP denotes the policy phase is separated by $\bar{d}_k$.}
