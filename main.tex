...
\begin{document}
    ...
    \begin{section}{ALGORITHM}
        ...
        \begin{subsection}{Stochastic Process for Baseline Policy}
            ...
            \end{align}
            where $T^{prop}_{k,n}$ only matters in \emph{cost collection} from each AP node but not with arrival distribution.

            Then given the initial states $S^{(C)}_{n}(t)$ for the servers as multi-queue structure:
            \begin{align}
                p^{(n)}(t) \triangleq \begin{bmatrix}
                    X_{L_C} \\ \vdots \\X_{1}
                \end{bmatrix}
            \end{align}
            where $X_{l}$ changes with time being follows the deterministic \emph{SRTF} rule.
