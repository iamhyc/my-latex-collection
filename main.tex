...
\begin{document}
    ...
    \begin{section}{SYSTEM MODEL}
        ...
        \begin{subsection}{Job Dispatching Model}
            ...
            Let $\uSet_{k,m}(t)$ denotes the jobs in uploading from $k$-th AP to $m$-th ES in $t$-th timeslot $\forall k \in \apSet, m \in \esSet$; $u \triangleq (l_u, T_{u,cd}(t)) \in \uSet_{k,m}(t)$, $l_u$ denotes the computing length on $m$-th server and $T_{u,cd}(t)$ records the remaining time for uploading completed.
            Let $\cSet_{m}(t)$ denotes the jobs waiting for computing on $m$-th ES in $t$-th timeslot; $c \triangleq (T_{c,cd}(t)) \in \cSet_{m}(t)$, $T_{c,cd}(t)$ for this job records the remaining time for computing completed.
            ...
            \begin{align}
                ...
                \\
                I^{(U \to C)}_{k,m}(t) &\triangleq \{ u | T_{u,cd}=1, \forall u \in \uSet_{k,m}(t)\}
                \\
                I^{(U \to U')}_{k,m}(t) &\triangleq \uSet_{k,m}(t) \backslash I^{(U \to C)}_{k,m}(t)
                \\
                I^{(C \to \Phi)}_{m}(t) &\triangleq \{ c | \arg\min_{c\in\cSet(t)} T_{c,cd}(t) \wedge T_{c,cd}=1\}
                \\
            ...
            \begin{align}
                \begin{cases}
                    ...
                    \\
                    \uSet_{k,m}(t+1) = I^{(W \to U)}_{k,m}(t) + I^{(U \to U')}_{k,m}(t)
                    \\
                    \cSet_{m}(t+1) = \sum\limits_{k\in\apSet} I^{(U \to C)}_{k,m}(t) + I^{(C \to C')}_{m}(t)
                \end{cases}
            \end{align}
            ...
            Additionally, we have the broadcast with period $T_{br}$
        \end{subsection}
    \end{section}
