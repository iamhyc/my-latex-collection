...
\begin{document}
    ...
    \begin{section}{PROBLEM FORMULATION}
        ...
        \begin{subsection}{The Optimization Problem}
            ...
            \begin{itemize}
                ...
                \begin{align}
                    p_{k,m}^{(i)}(j) \define& \lambda_k p(j) \cdot
                        \frac{
                                \Pr\{\omega_{k,m}^{(i)}(j)\}
                            }{
                ...
                \end{align}
                and we have $p_{k,m}^{(\Pi_k)}$ denotes the distribution under time-invariant policy $\Pi_k$ for $k$-th AP;
    ...
    \begin{section}{LOW-COMPLEXITY SOLUTION}
        ...
        \begin{subsection}{Baseline Dispatching Policy}
            ...
            \end{align}
            where $\tilde{W}^{(p)}(\cdot)$ and $\tilde{W}^{(s)}(\cdot)$ denote the split value function over AP nodes and ES nodes respectively; $\Obsv^{(p)}(i) \define \set{\vec{n}_k(i)|\forall k\in\apSet}$ and $\Obsv^{(s)}(i) \define \set{\vec{Q}_m(i)|\forall m\in\esSet}$ respectively denote the AP states collection and ES states collection for $\Obsv(i) = \set{\Obsv^{(p)}(i), \Obsv^{(s)}(i)}$, and the split value function is in the following form:
            \begin{align}
            ...
            \begin{align}
                \tilde{W}^{(p)}(\Obsv^{(p)}(i+1)) = \frac{1}{1-\gamma}
                    \sum_{k\in\apSet}\sum_{m\in\esSet} \mathbb{E}_{\Pi_k}[\Inorm{\vec{r}^{(k)}_{m}}],
            \end{align}
            where the expectation of $\Inorm{\vec{r}^{(k)}_{m}}$ under the policy $\Pi_k$ is actually a constant as expectation of Binomial distribution $\mathbb{E}_{\Pi_k}[\Inorm{\vec{r}^{(k)}_{m}}] = \sum_{j\in\jSpace} u_{k,m} p^{(\Pi_k)}_{k,m}(j)$.
            ...
            \end{align}
            Let $\tilde{V}^{\Policy(\Stat_i)}(Q_{m,j}(i+1)) \define \mathbb{E}_{\Obsv^{(p)}(t+1)|\Policy(\Stat_t)} [\tilde{W}^{(s)}_{m,j}(\Obsv(i+1))]$ denotes the averaged value function for $m$-th ES node, which implies that the state $\Obsv^{(p)}(i)$ is substitute with its expectation under policy $\Policy(\Stat_i)$ in this value function. Thus the approximated value function for $m$-th ES node is denoted as:
            \begin{align}
                \tilde{V}\bigg(Q_{m,j}(i+1)\bigg) \define& \lim_{T\to\infty}
            ...
            \end{align}
            where $\vec{\mu}'_i$ denotes the transpose of $\vec{\mu}_i$, and $\vec{\mu}_i = [0,\dots,0,1,\dots]$ with only $i$-th element as $1$; the $i$-th element of $\vec{g}$ denotes the cost of server as $n_{m,j}(i)$ for $i$-th stage; $\mat{P}_{m,j}$ denotes the transition matrix for $j$-type FIFO queue on $m$-th ES under the policy $\vec{\Pi}$, which is composed of the following transition function ($\forall Q_m(i),Q_m(i+1)$) as:
            \begin{align}
                & \Pr \Brace{Q_{m,j}(i+1)|Q_{m,j}(i),\vec{\Pi}}
                \nonumber\\
                =& \prod_{k\in\apSet} \Pr\{\tilde{n}_{m,j}(i)\} \times I[ \tilde{n}_{m,j}(i)=n^{(k)}_{m,j}(i+1) ]
                    \nonumber\\
                    &\times I[\tilde{n}_{m,j}(i) f(j)^{(m)} = t_B+\delta_{m,j}(i+1)-\delta_{m,j}(i)],
            \end{align}
            where:
            \begin{itemize}
                ...
                \item $\tilde{n}_{m,j}(i) = \sum_{k\in\apSet} \tilde{n}^{(k)}_{0} + \tilde{n}^{'(k)}_{m,j} + n_{m,j}(i)$, where $\tilde{n}^{(k)}_{0} = u_{k,m}p^{(\lambda,i)}_{k,m}$ is the averaged $\Obsv^{(AP)}$ state under policy $\Policy(\Stat_i)$, $\tilde{n}^{'(k)}_{m,j} \sim Bin(t_B-u_{k,m}, p^{(\lambda, \Pi_k)}_{k,m}(j))$, $(\forall k\in\apSet)$.
            \end{itemize}
        \end{subsection}
        ...
        \begin{subsection}{The Distributed Algorithm}
            ...
            \begin{align}
                ...
                \nonumber\\
                \times \bigg[& \sum_{m\in\esSet}\sum_{j\in\jSpace} \tilde{V}^{\Policy(\Stat_i)}\bigg(Q_{m,j}(i+1)\bigg) +
                    \nonumber\\
