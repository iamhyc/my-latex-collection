...
\begin{document}
    ...
    \begin{section}{PROBLEM FORMULATION}
        ...
        \begin{subsection}{The Optimization Problem}
            ...
            The Categorical distribution in each timeslot consists of the multinomial distribution for number of jobs on AP nodes or on ES nodes in one broadcast interval.
            
            With the help of multinomial distribution arrival process, we could further decouple the expression of transition function into parts 
            \begin{lemma}[Transition Function Decoupling]
                ...
                \begin{align}
                    ...
                    \nonumber\\
                    =& \prod_{k\in\apSet} \Pr\Brace{\vec{R}_k(i+1)|\BPolicy(\Stat_{i})} \times \prod_{j\in\jSpace}
                        \nonumber\\
                        & \Pr\Brace{\vec{Q}'_{j}(i+1)|\vec{Q}'_{j}(i), \set{\vec{R}_k(i)|\forall k\in\apSet}, \BPolicy(\Stat_{i})},
                \end{align}
                where $\vec{Q}'_{j}(i)\define \set{Q_{m,j}(i)|\forall m\in\esSet}$; first production part denotes the unfinished uploading state transition for AP nodes, and the second part denotes the queueing states of same type jobs on all ES nodes.
            \end{lemma}
            \begin{proof}
    ...
    \begin{section}{LOW-COMPLEXITY SOLUTION}
        ...
        \begin{subsection}{Baseline Dispatching Policy}
            ...
            \end{align}
            where the expectation of $\Inorm{\vec{r}^{(k)}_{m}}$ under the policy $\Pi_k$ is actually a constant as expectation of Binomial distribution $\mathbb{E}_{\Pi_k}[\Inorm{\vec{r}^{(k)}_{m}}] = \sum_{j\in\jSpace}\sum_{m\in\esSet} \bar{u}_{k,m} \rho^{(\Pi_k)}_{k,j}(m)$.
    ...
    \begin{section}{Transition Function Decoupling}
        ...
        \begin{align*}
            &\Pr\{\vec{R}_k(i+1)|\BPolicy(\Stat_i)\} =
            % &\vec{r}^{(k)}_{j}(i+1) \sim \Mult\Paren{u_{k,m},p^{(i+1,0)}_{k,j}},
        \end{align*}
