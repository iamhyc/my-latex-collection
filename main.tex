        ...
        \begin{subsection}{Problem Formulation}
            ...
            \begin{subsubsection}{Single-Step Transition}
                ...
                \begin{align}
                    ...
                    \\
                    I^{(C \to \Phi)}_{n}(t) &\triangleq \{j|\arg\min_{j} \{(L^{(C)}_{cd})_j=1\} \}
                    \\
                ...
                \begin{align}
                    \begin{cases}
                        ...
                        \\
                        S^{(C)}_{k,n}(t+1) = I^{(U \to C)}_{k,n}(t) + I^{(C \to C')}_{n}(t)
                    \end{cases}
                \end{align}
                and we note that only $I^{(W \to W')}_{k}(t)$ and $I^{(W \to U)}_{k,n}(t)$ are with disturbance from arrival process and randomized policy. We could obtain the joint distribution of independent AP agents with respect to $S^{(W)}(t),S^{(U)}(t)$ as:
                \begin{align}
                    & \Pr\{
                        \begin{pmatrix}
                            S^{(W)}(t+1) \\ S^{(U)}(t+1)
