...
\begin{document}
    ...
    \begin{section}{LOW-COMPLEXITY SOLUTION}
        ...
        \begin{subsection}{Baseline Dispatching Policy}
            ...
            \begin{align}
                J\bigg(Q_{m,j}(i+1)\bigg) \define& \lim_{T\to\infty}
        ...
        \begin{subsection}{The Distributed Algorithm}
            ...
            \begin{align}
                ...
                \nonumber\\
                \times \Bracket{
                    & \sum_{m\in\esSet}\sum_{j\in\jSpace} J\bigg(Q_{m,j}(i+1)\bigg)
                } +
                \nonumber\\
                & \underbrace{\sum_{\Obsv^{(p)}(i+1)} \Pr\{\Obsv^{(p)}(i+1)|\Policy(\Stat_i)\}}_{\text{=1}}
                \cdot \underbrace{\tilde{W}^{(p)}\bigg(\Obsv^{(p)}(i+1)\bigg)}_{\text{constant}}
            \end{align}
