...
\begin{document}
    ...
    \begin{section}{SYSTEM MODEL}
        ...
        \begin{subsection}{Network Model}
            ...
            {\color{red}The corresponding uploading delay of one job is deterministic and job-type dependent over one AP-ES link, which is denoted as $u_{k,m}(j)$ from $k$-th AP to $m$-th ES ($\forall k\in\apSet, \forall m\in\esSet, \forall j\in\jSpace$).}
        ...
        \begin{subsection}{Information-Sharing Broadcast Model}
            ...
            \begin{definition}[Update Latency]
                ...
                \end{align}
                where $D^{(p)}_{k,k'} \equiv 0$ for convenience. {\color{red}We assume that $D_k$ is bounded with $[0, \hat{D}_k]$ and follows some distribution in this interval summarized from statistical data, which is denotes as $\eta_k(d) \define \Pr\{D_k=d\}$, with mean denoted as $\bar{d}_k$.
                And we have $\vecG{\eta} \define \set{\eta_k|\forall k\in\apSet}$ to denote the latency distribution set for all AP nodes.}
            \end{definition}
            ...
            {\color{red}Moreover, we implement the broadcast in a low-frequency way that the broadcast period is always larger than the maximum broadcast latency plus the maximum uploading delay, i.e. $t_B > \hat{D}_{k} + u_{k,m}(j)$ $(\forall k\in\apSet, \forall m\in\esSet, \forall j\in\jSpace)$.}
    ...
    \begin{section}{PROBLEM FORMULATION}
        ...
        \begin{subsection}{System State and Dispatching Policy}
            ...
            \end{definition}
            More specifically, the two phase of the policy on single AP is separated by the \emph{Update Latency} $D_k$ for $k$-th AP, i.e. the exact time point when it receives the latest global information.
            For example, in the broadcast interval $[t_{i}, t_{i+1}]$, $k$-th AP has the exact update latency as $d_k$, then it will apply policy $\tilde{\Omega}_k(\Obsv_{i})$ based on obsolete information before $(t_i+d_k)$ and apply the updated policy $\tilde{\Omega}_k(\Obsv_{i+1})$ immediately by solving the optimization problem with prior statistical knowledge $\vecG{\eta}$.
        \end{subsection}
        ...
        \begin{subsection}{The Optimization Problem}
            ...
            \begin{align}
                V(\Stat_i) =& g(\Stat_i) +
                    \gamma \min_{\Policy(\Stat_{i})} \sum_{\Stat_{i+1}} \sum_{k\in\apSet}\sum_{d_k} \eta_k(d_k) \cdot
                    \nonumber\\
            ...
            \end{align}
            {\color{red}where $\BPolicy(\Stat_{i}) \define [\bar{\Omega}_1(\Stat_{i}), \dots, \bar{\Omega}_K(\Stat_{i})]$; each $\bar{\Omega}_k(\Stat_{i})$ on $k$-th AP denotes the policy phase is separated by $\bar{d}_k$.}
            ...
            \begin{lemma}[Transition Function Decoupling]
                ...
                \begin{align}
                    ...
                    =& \prod_{k\in\apSet} \Pr\Brace{\vec{R}_k(i+1)|\BPolicy(\Stat_{i})} \times \prod_{m\in\esSet}\prod_{j\in\jSpace}
                        \nonumber\\
                        & \Pr \Brace{\vec{Q}_{m,j}(i+1)|\vec{Q}_{m,j}(i), \set{\vec{R}_k(i)|\forall k\in\apSet}, \BPolicy(\Stat_{i})},
                \end{align}
    ...
    \begin{section}{LOW-COMPLEXITY SOLUTION}
        ...
        \begin{subsection}{Baseline Dispatching Policy}
            ...
            \end{align}
            where the expectation of $\Inorm{\vec{r}^{(k)}_{m}}$ under the policy $\Pi_k$ is actually a constant as expectation of Binomial distribution $\mathbb{E}_{\Pi_k}[\Inorm{\vec{r}^{(k)}_{m}}] = \sum_{j\in\jSpace} u_{k,m}(j) \rho^{(\Pi_k)}_{k,m}(j)$.
    ...
    \begin{section}{Transition Function Decoupling}
        ...
        \begin{align*}
            &\Pr\{\vec{R}_k(i+1)|\BPolicy(\Stat_i)\} = \prod_{m\in\esSet}\prod_{j\in\jSpace} r^{(k)}_{m,j}(i+1)
            \\
