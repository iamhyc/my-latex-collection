...
Firstly, we notice that the transition function in equation (\ref{eqn:sp_0}) could be rewrite in the following form.
    \begin{align}
    ...
    \end{align}
    where the transition function is decoupled into two parts of state transition on APs and edge servers, respectively, under policy $\Policy$.
