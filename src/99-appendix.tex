...
Moreover, we notice that under the fixed baseline policy, the arrival process on edge servers would be stationary after the \emph{maximum uploading time} from the initial interval and thus the transition matrix is invariant of system states of APs.
...
% \section{ Proof of Lemma \ref{lemma:w_partial} }
% \label{append_3}
...
where $g_{\max}$ denotes the upper-bound of the per-stage cost.

Because  $\mathfrak{F}(N)=\sum_{t=1}^{N}\gamma^{t-1}\Delta(t)\geq 0$, $\mathfrak{F}(N)$ increases as $N$ increases.
It is obvious that $\mathfrak{G}(N)$ decreases as $N$ increases, and 
\begin{align*}
