...
\accept{
    We consider the edge computing network with APs and edge servers residing in the Metropolitan Area Network (MAN).
    The APs shall collect jobs offloaded from the mobile users in its range and make dispatching decisions for each job on the edge servers.
    The existing literature, such as \cite{tan-online,MOBIHOC19-ZhouZ,IOTJ18-FanQ,TOC19-LiuC,JSAC19-AlameddineHA}, considers the transmission latency of jobs is non-negligible but fixed in MAN.
    However, according to one MAN performance analysis research in \cite{MAN-LATENCY}, the transmission latency will vary a lot with respect to different hours of day and devices' locations in MAN.
...
% There are also some works considering decentralized multi-agent scheduling where the information exchange scheme matters.
