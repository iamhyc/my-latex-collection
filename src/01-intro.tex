...
For example, the authors in \cite{JSAC17-LyuX} proposed a randomized policy via Lyapunov optimization approach to stabilize the queues \fixit{in a xx system with what}, where \brlatency~is considered. 
In \cite{TWC18-LyuX}, the mobile devices offload jobs selfishly with the stale information.
\fixit{The above approach is extended to the scenario that what.}
