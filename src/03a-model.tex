...
As a high frequency broadcast design would always result into \emph{broadcast storm} and block the normal network traffic, we introduce a fair-slow enough period selection
The broadcast interval is always larger than the maximum \brdelay, i.e. $t_B > \hat{D}_k$ ($\forall k\in\apSet$), where $\hat{D}_k$ is the upper bound for $D_{k}(t_{i})$ ($i=1,2,\dots$).
\fixit{
