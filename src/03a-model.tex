...
\end{align}
where we have $\vec{R}^{(k)}_{m,j} \define ( R^{(k)}_{m,j,0},\dots,R^{(k)}_{m,j,\Xi} )$ denotes the vector of random variables for convenience.
...
\begin{align}
    &\tilde{\Omega}_{k}\Paren{\Obsv({t})} \define \set{\omega_{k,j}({t})|\forall m\in\esSet, j\in\jSpace}
    ~(\forall k\in\apSet).
    \label{def_action}
\end{align}
