...
\comment{
    Due to the limitation of network topology, each AP could only dispatch the computation jobs to a subset of edge servers, which is denoted as \emph{candidate set} $\mathcal{M}_{k} \subseteq \esSet$ ($\forall k\in\apSet$).
}
