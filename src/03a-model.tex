...
\begin{definition}[Local State Information of APs]
    ...
    \begin{align}
        \mathcal{R}_{k}(t) \define \Brace{
            \big( \vec{R}^{(k)}_{m,j}(t,0), \omega_{k,j}(t) \big) | \forall m\in\esSet_{k}, j\in\jSpace
        },
    \end{align}
    where
...
\begin{definition}[Global State Information]
    ...
    \begin{align}
        \Stat(t) \define
            \Brace{
                \big( \mathcal{R}_{k}(t), \mathcal{Q}_{m}(t) \big)| \forall k\in\apSet, m\in\esSet
            }.
    \end{align}
\end{definition}
