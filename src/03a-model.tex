...
\fixit{
    Specifically, let $\omega_{k,j}(t)$ denote the computing edge server at the first half time of the $t$-th broadcast interval, the $k$-th AP should determine the new policy $\omega_{k,j}(t)$ in the $D_{k}(t)$ time slot of the $t$-th broadcast interval, which is denoted as $\omega_{k,j}(t+1)$ because it would broadcast the policy at the $(t+1)$-th broadcast.
}
Hence, we denote the individual dispatching policy of the $k$-th AP based on the GSI $\Obsv^{\dagger}(t)$ as follows.
\begin{align}
    &\Omega_{k}(\Obsv^{\dagger}(t)) \define \set{\omega'_{k,j}(t)|\forall m\in\esSet, j\in\jSpace}.
    \label{def_action}
\end{align}
\fixit{
...
\fixit{
    (Stupid narrative! There are no such naive readers!)\\
    However, the randomness of \brlatency~implies that one AP could not know other APs' \brlatency~in the same interval.
    ...
    In the following problem formulation section, we will show that we could come up with better dispatching decision update solution which is aware of the \brlatency, and improve APs' dispatching decisions in an iterative way.
