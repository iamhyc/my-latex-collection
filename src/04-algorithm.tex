...
\begin{enumerate}
    ...
    \item Based on the approximate value function, an alternative action update algorithm, where a subset of APs are selected to update their dispatching action distributedly in each broadcast interval, is proposed to solve the RHs of the Bellman's equation in \ref{eqn:sp_0} in Section \ref{subsec:ap_alg}.
    Moreover, the analytically performance bound is also derived.
\end{enumerate}
\wangr{As a remark notice that solving the value function and then the RHS of the Bellman's equations are the two standard steps to solve an MDP problem with complete knowledge on system state.
...
Instead, it is feasible for part of APs to update their dispatching actions distributedly and achieve a better performance compared with baseline policy.
...
The optimization of $\tilde{\mathcal{A}}_{y}(t)$ for the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$) in P3 could be achieved via searching all the edge servers in $\esSet_{y}$, whose computational complexity is $O(JMK)$ per AP. % for evaluating the objective function in equation (\ref{eqn:partial}).
As a result, the overall algorithm of job dispatching is elaborated in Algorithm \ref{alg_1}.
\begin{algorithm}[ht]
    \caption{Online Alternative Actions Update Algorithm}\label{alg_1}\tann{(line-by-line explanation of the algorithm is necessary.)}
...
\subsection{Theoretical Analysis\tann{(tann)}}
...
\end{proof}
\tann{(Give an explanation briefly what this upper bound means. ) Therefore, The cost? of our algorithm is upper bounded by ... , which means...}
Therefore, the performance of the proposed algorithm is upper bounded by $W_{\Baseline}(\Stat)$ ($\forall \Stat,t$) which means it is always better than baseline policy. 
