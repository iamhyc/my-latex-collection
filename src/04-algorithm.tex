...
\delete{
    \begin{itemize}
        ...
        For example, in the $i$-th broadcast interval, the $k$-th AP would update its policy $\tilde{\Omega}_{k}(\Obsv(t_{i}))$ according to the $k$-th sub-problem while the other AP nodes would keep with the previous policy $\tilde{\Omega}_{k'}(\Obsv(t_{i-1}))$($\forall k' \neq k \in\apSet$).
