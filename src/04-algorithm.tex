...
\subsection{Distributive Action Update}
\label{subsec:ap_alg}
...
The optimization of $\tilde{\mathcal{A}}_{y}(t)$ for the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$) in P3 could be achieved via searching all the edge servers in $\esSet_{y}$, whose computational complexity is $O(J(KM+M))$ per AP. % for evaluating the objective function in equation (\ref{eqn:partial}).
...
\begin{algorithm}[ht]
    ...
    \For{$t=0,1,2,\dots$}{
        ...
        \ForPar{$y \in \mathcal{Y}_{n}$}{
            Observe $\Stat_{y}(t)$ after $\mathcal{D}_{y}(t)$ at the $y$-th AP.\;
...
As a remark notice that Algorithm \ref{alg_1} leads to a time-variant policy, which is denoted as $\tilde{\Policy}$.
...
Since the AP subsets are selected periodically, we have $\tilde{\Policy}(\Stat, t) = \tilde{\Policy}(\Stat, t+N)$.
...
\end{lemma}
\begin{proof}
    \fixit{Policy Improvement in \cite{dp-control}.}
\end{proof}
