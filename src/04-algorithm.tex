...
\fixit{
    Due to the limit of OSI, we update the policy of each AP independently.
    Specifically, there is always one and only one AP updating the policy in one interval, while the other APs keep the policy fixed as broadcast information for the AP in updating.
    The $k$-th AP would update its policy only with it OSI required.
    We consider the broadcast and define the new problem for the $k$-th AP as follows.
