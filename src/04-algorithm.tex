...
\begin{align}
    \hat{\mathcal{A}}(t) \define \tilde{\mathcal{A}}(t) \cup \set{\Pi_{y}(\Stat(t)) | \forall y\notin\mathcal{Y}_{n}}
\end{align}
...
\begin{align}
    ...
    &\sum_{\Stat(t+1)} \Pr\Brace{
        \Stat(t+1) | \Stat(t), \hat{\mathcal{A}}(t)
...
\accept{
    \begin{enumerate}
        ...
        \item Each AP (say the $y_0$-th AP) in set $\mathcal{Y}_{0}$ would receive the OSI at the $D_{y_0}(1)$ time slots in the $1$-st broadcast interval, and then it updates its dispatching actions $\tilde{\mathcal{A}}_{y_0}(1)$ by solving P3;
        ...
        \item Similarly, in the $t$-th broadcast interval the APs in subset indexed with $n \define (t+1)\mod{N}$ would receive the OSI, and then update their dispatching actions $\tilde{\mathcal{A}}_{y_n}(t)$ ($\forall y_n\in\mathcal{Y}_{n}$) by solving P3.
        \item Step 4) is periodically repeated with respect to the broadcast interval.
    \end{enumerate}
}
% \begin{remark}[Complexity Analysis]
%     The computation complexity of solving Problem P3 is $O(J())$

%     % The computation complexity of solving Problem P3 is $O(J(c_k M_k+M_k))$ where $c_k, M_k$ is the number of APs in the \emph{conflict AP set} and number of edge servers in the \emph{potential server set} of the $k$-th AP, respectively.
...
\begin{lemma}[Performance Guarantee]
    ...
    $\tilde{\Policy}(\Stat(t)) \define \hat{\mathcal{A}}(t)$,
    \begin{align}
