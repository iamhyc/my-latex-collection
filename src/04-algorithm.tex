...
\end{enumerate}
\comments{
    More specifically, step 1 will be introduced in subsection \ref{subsec:baseline} and step 2 will be introduced in subsection \ref{subsec:ap_alg}.
}
...
\comments{
    ...
    Specifically, the baseline policy used in $\Stat(t+1)$ is the policy taken in the last interval, i.e. $\Omega(\Stat(t), \Delay(t))$.
...
\end{align}
\comments{
    However, the OSI is not always available to all the APs, and the reception of OSI may result into unacceptable latency.
    Hence, we leverage the nature of 
}
...
\comments{
    ...
    \begin{lemma}[Decouple Problem \ref{problem_2}]
