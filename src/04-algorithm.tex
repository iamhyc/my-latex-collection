...
Hence, in the $t$-th broadcast, the APs in the subset indexed with $n \equiv t \pmod{N} + 1$ should update their dispatching actions.
Moreover, we shall adopt the baseline policy $\Pi_{t,y}$ for the $y'$-th APs $\forall y' \notin \mathcal{Y}$.
\fixit{
    Hence, let $xx=xx$ be the aggregation of dispatching actions for the APs in the subset $\mathcal{Y}_{n}$, and $xx=xx$ be the aggregation of dispatching actions of all APs.
    ...
    %         \\
    %         \Pi_{y}(\Stat_{y}(t)), & \text{otherwise.}
    %     \end{cases}
    % \end{align}
}

The optimization of the dispatching actions $\Policy$ at the RHS of the Bellman's equations can be rewritten as the following problem.
...
\fixit{
    (DELETE)
    The GSI is not necessary in solving P2 when the alternative dispatching policy is considered, because the update policy would only change the cost raised on local AP and the corresponding \emph{candidate server set}.
    We could simply reduce the expression of equation (\ref{w_ap}) and equation (\ref{w_es}) on the RHS of the Bellman's equations based on local OSI.
    Based on the above lemma, we could rewrite the P2 as the following reduced form.
}

\comments{
    The optimization of $\tilde{\Omega}_{y}(\Stat_{y}(t), \Delay_{y}(t))$ could be applied by linear search over the solution space of size $|\esSet_{y}|$ for the $y$-th AP ($\forall y\in\mathcal{Y}_{n}$), and then evaluating equation (\ref{eqn:partial}) for each possible solution.
