...
The \brlatency~is with the support $[10,16]$ within one broadcast interval.
...
\hongyc{It is shown that the proposed policy can process the approximately the same number of jobs as Queue-aware Policy and Random Policy, while with the minimum average cost and job response time.}
...
\begin{figure*}[ht!]                                                                %
    ...
    \begin{tabular}{ccc}                                                            %
        ...
        {\small (b) Signaling latency with Support $[10,16]$} &
...
The simulation results with various supports with \brlatency~$\mathcal{D}_{k}$ ($\forall k\in\apSet$) are illustrated in Fig.\ref{fig:ss_delay}, where the cumulative distribution function (CDF) of the job number in the system is plotted.
Specifically, the \brlatency~of all the APs is set to zero and $20$ in Fig.\ref{fig:ss_delay}(a) and Fig.\ref{fig:ss_delay}(c), respectively, and the \brlatency~is a random variable with support $[10,16]$ in Fig.\ref{fig:ss_delay}(b).
It can be observed from Fig.\ref{fig:ss_delay}(a) and Fig.\ref{fig:ss_delay}(c) that, with the increasing \brlatency, the performance of Queue-aware Policy becomes worse.
...
In all the figures, the proposed policy performs better than the benchmarks, which demonstrate the robustness of its performance versus signaling latency.
...
The simulation results are consistent with Fig.\ref{fig:ss_scale}.
