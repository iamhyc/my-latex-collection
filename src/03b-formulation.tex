...
\begin{definition}[Dispatching Policy]
    The individual policy of the $k$-th AP $\Omega_{k}$ ($\forall k \in\apSet$) maps from its OSI $\Stat_{k}$ and its \emph{delivery latency} $\mathcal{D}_{k}$ to the dispatching strategy for each job type, i.e.,
    \begin{align}
