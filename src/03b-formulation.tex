...
\fixit{
    which considers optimization of global dispatching decision over the global states which is compounded of all AP and edge servers.
}
Given the fact that AP nodes would only update their own global information only after \brdelay in each broadcast interval, the state and policy in formulated MDP problem are actually composed of both stale and updated information \fixit{due to that delay}.
