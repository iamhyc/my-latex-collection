...
%----------------------------------------------------------------------------------------%
In this section, we formulate the optimization problem under standard MDP framework, which aims at online optimizing dispatching decisions of all APs.
Given that each AP makes decisions based on collected global state information, we describe the system state and policy from a global perspective, where each AP adopt its dispatching policy mapping from the global system state.

However, due to the different distributions of \brdelay~for each AP, APs update their policy adoption at different time slots in an unknown order \fixit{which makes it impossible to optimize all the APs at once}.

\fixit{
    To overcome the gap, we have to distributed optimize the dispatching policy of each AP independently.
...
The system states is selected that $k$-th AP comes up with complete broadcast information only after its corresponding \brdelay in the broadcast interval.
...
% Furthermore, the update is applied iteratively, as the name implies, with the order following length of broadcast delay (i.e. the sorted AP nodes order). The corresponding Bellman's Equation for $k$-th AP ($\forall k\in\apSet$) node is defined as follows.
% \begin{align}
