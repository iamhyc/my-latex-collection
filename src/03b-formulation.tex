...
Therefore, we further decouple the global problem into \fixit{$K$ sub-problems} to cater for the real decentralized situations \delete{how the system state is updated for each AP}.
...
Based on the two phases of global broadcast information illustrated in system state, the global compounded policy of all APs is defined as follows.
...
To overcome the gap, we leverage the above global problem structure, but develop \fixit{$K$-sub MDP problems} where the APs update their own policy independently in a decentralized manner.
...
\begin{align}
    ...
    \nonumber\\
    &~~~~+ \gamma \min_{\tilde{\Omega}_{k}(\Obsv(t_{i}))} \Brace{ \Obsv(t_{i+1})|\Obsv(t_{i}), \Policy'_{k}(\Stat(t_{i})), D_{k}(t_{i}) } \cdot V\Paren{\Stat(t_{i+1})}.
    \label{sp_k}
\end{align}
% $\tilde{\Omega}_{k}(\Obsv(t_{i}))$ is the component of $\Policy(\Obsv(t_{i}))$.
\begin{remark}
    Assume that we solve the sub-problems following the order of index of AP set, and substitute the solution to the $k$-th problem to the $(k+1)$-th problem ($\forall k\in\apSet$).
...
\begin{lemma}[Transition Function Decoupling]
    ...
    \begin{align}
        & \Pr\{\Obsv(t_{i+1})|\Obsv(t_{i}), \Policy(\Stat(t_{i})), D_{k}(t_{i})\}
        \nonumber\\
        =& \prod_{k\in\apSet}\prod_{m\in\esSet}\prod_{j\in\jSpace}
                \Pr\Brace{
                    \vec{r}^{(k)}_{m,j}(t_{i+1})|\vec{r}^{(k)}_{m,j}(t_{i}), \Policy(\Stat(t_{i})), D_{k}(t_{i})
                }
                \times  
            \nonumber\\
            & \prod_{m\in\esSet}\prod_{j\in\jSpace}
                \Pr\Brace{
                    Q_{m,j}(t_{i+1})|Q_{m,j}(t_{i}), \mathcal{R}(t_{i}), \Policy(\Stat(t_{i})), D_{k}(t_{i})
                }.
    \end{align}
\end{lemma}
\begin{proof}
