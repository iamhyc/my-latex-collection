...
\delete{
    Outline:
    \begin{itemize}
        \item We formulate the multi-agent MDP problem where each AP shares same utility function in a fully cooperative setting and individually takes action to perform.
...
\accept{
    Due to the periodic information broadcasting, we collect the cost for counting numbers at the pace of broadcast interval which could be seen as a uniform sampling of original process based on timeslot scale.
}
