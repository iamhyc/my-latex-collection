...
The transition function, except for standalone distribution of $\Pr\{\mathcal{D}({t})\}$ falls into two production part.
\fixit{
...
\begin{align}
    \Pr\Brace{
        \vec{R}^{(k)}_{m,j}({t+1}) | \vec{R}^{(k)}_{m,j}(t)
    } \sim \vecG{\Theta}^{(k)}_{m,j}({t+1}),
\end{align}
where $\vecG{\Theta}^{(k)}_{m,j}({t+1}) \define [\vecG{\theta}^{(k)}_{m,j,0}({t+1}), \dots, \vecG{\theta}^{(k)}_{m,j,\Xi}({t+1})]$.
Given that the uploading process is depicted by a series of counters, we further have
$\Pr\Brace{R^{(k)}_{m,j,\xi+1}({t+1})|R^{(k)}_{m,j,\xi}({t})} \sim \vecG{\theta}^{(k)}_{m,j,\xi}({t+1})$ ($\forall \xi=0,\dots,\Xi$).
Then we could express the distribution with a transition matrix as:
\begin{align}
    \vecG{\Theta}^{(k)}_{m,j}({t+1}) \define \hat{\Gamma}^{(k)}_{m,j}\Paren{\Policy(\Stat({t})) }\vecG{\Theta}^{(k)}_{m,j}({t}),
\end{align}
...
\begin{align}
    ...
    \end{cases},
\end{align}
where $\vecG{\theta}^{(k)}_{m,j,0}(t_{i}) \sim \text{Bernoulli}(\tilde{\lambda}^{(k)}_{m,j}(t_i))$. The transition matrix $\Gamma^{(k)}_{m,j,\xi}$ is time-invariant and the definition is given as follows.
\begin{align}
    \Gamma^{(k)}_{m,j,\xi} = 
    \begin{bmatrix}
        &0 \\
