...
However, it is infeasible to solve the above Bellman's equations in our problem \wangr{and achieve the performance of $\Policy^*$ in our problem.}
...
The general solution of POMDP is of huge complexity \wangr{as all the historical system states should be involved in the current system state} \cite{IJCAI03-NairR,IJCAI99-BoutilierC}.
In this paper, we shall propose a novel low-complexity solution framework based on an analytical approximation of the value function $V(\cdot)$ and alternative action update, where distributive job dispatching via the Bellman's equations becomes feasible even with OSI and local signaling latency.
%----------------------------------------------------------------------------------------%
