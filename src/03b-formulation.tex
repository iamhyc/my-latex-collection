...
For state transition of APs, we denote $\vecG{\Theta}^{(k)}_{m,j}$ and $\dot{\Gamma}^{(k)}_{m,j}$ as probability vector and transition matrix, respectively, for $\vec{R}^{(k)}_{m,j}$ of the $j$-th type of job uploading from the $k$-th AP to the $m$-th edge server ($\forall k\in\apSet, m\in\esSet, j\in\jSpace$). The definition is given as follows.
\begin{align}
...
\fixit{
    And we notice that $\Pr\{R^{(k)}_{m,j,0}\}(t,n)$ is determined simply by arrival process and dispatching policy which motivates the system to work.
    \\
