...
\begin{align}
    ...
    \end{bmatrix},
\end{align}
where $p^{(k)}_{m,j,\xi} \define \Pr\{U^{(k)}_{m,j} < (\xi+1) | U^{(k)}_{m,j}>\xi\}$.

For state transition of APs, we denote $\vecG{\Theta}^{(k)}_{m,j}$ and $\dot{\Gamma}^{(k)}_{m,j}$ as probability vector and transition matrix, respectively, for $\vec{R}^{(k)}_{m,j}$ of the $j$-th type of job uploading from the $k$-th AP to the $m$-th edge server ($\forall k\in\apSet, m\in\esSet, j\in\jSpace$).
The probability vector denotes the ???
\fixit{
    \begin{itemize}
        \item definition for $\vecG{\Theta}^{(k)}_{m,j}$, and the conversion function given $\Policy(\Stat(t))$;
        \item definition for $\Gamma^{(k)}_{m,j}$ (not block matrix any more);
        \item Given that the uploading process is depicted by a series of counters \dots
    \end{itemize}
}
\begin{gather*}
...
where transition matrix $\dot{\Gamma}^{(k)}_{m,j}$ is a \emph{block matrix} whose element is transition matrix $\Gamma^{(k)}_{m,j,\xi}$ for the distribution on the series of counters. The definition is given as follows.
\begin{align}
...
\fixit{
    \begin{itemize}
        \item explain the little probability approximation;
        \item definition of $\vecG{\mu}_{m,j}$
    \end{itemize}
}
Denote as
\begin{align}
